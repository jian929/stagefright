...
OMX_ERRORTYPE OMXComponentCodecBase::ComponentGetParameter(
    ...
    if (handler == NULL) {
        ALOGE("ComponentGetParameter: No handler for index %d", nIndex);
        return OMX_ErrorUnsupportedIndex;
    }

    ALOGV("ComponentGetParameter: Index = 0x%x", nIndex);
...
OMX_ERRORTYPE OMXComponentCodecBase::ComponentSetParameter(
    ...
    if (handler == NULL) {
        ALOGE("ComponentSetParameter: No handler for index %d", nIndex);
        return OMX_ErrorUnsupportedIndex;
    }

    ALOGV("ComponentSetParameter: Index = 0x%x", nIndex);
    return (*handler)(this, pComponentParameterStructure);
}
...
OMX_ERRORTYPE OMXComponentCodecBase::ComponentGetConfig(
    ...
    OMXHANDLER handler = FindHandler(nIndex, true);
    if (handler == NULL) {
        ALOGE("ComponentGetConfig: No handler for index %d", nIndex);
        return OMX_ErrorUnsupportedIndex;
    }

    ALOGV("ComponentGetConfig: Index = 0x%x", nIndex);
...
OMX_ERRORTYPE OMXComponentCodecBase::ComponentSetConfig(
    ...
    OMXHANDLER handler = FindHandler(nIndex, false);
    if (handler == NULL) {
        ALOGE("ComponentSetConfig: No handler for index %d", nIndex);
        return OMX_ErrorUnsupportedIndex;
    }

    ALOGV("ComponentSetConfig: Index = 0x%x", nIndex);
...
OMX_ERRORTYPE OMXComponentCodecBase::ProcessorInit(void) {
    ALOGV("OMXComponentCodecBase::ProcessorInit");
