...
{
    ALOGV("OMXVideoDecoderVP9HWR is constructed.");
...
//#ifdef DECODE_WITH_GRALLOC_BUFFER
    ...
    *mDisplay = ANDROID_DISPLAY_HANDLE;
    ...
    if (vaStatus != VA_STATUS_SUCCESS) {
        ALOGE("vaInitialize failed.");
    } else {
        ALOGV("va majorVersion=%d, minorVersion=%d", majorVersion, minorVersion);
    }
#endif
}
...
OMXVideoDecoderVP9HWR::~OMXVideoDecoderVP9HWR()
{
    ALOGV("OMXVideoDecoderVP9HWR is destructed.");
                          ...
                          vpx_codec_frame_buffer_t *fb)
{
    ...
    if (extNativeBufferSize < new_size) {
        ALOGE("Provided frame buffer size < requesting min size., new_size is %d, extNativeBufferSize is %d\n", new_size, extNativeBufferSize);
        return -1;
    }

    int i;
    for (i = 0; i < extMappedNativeBufferCount; i++ ) {
        ALOGV("0 buffer %d virtual address is 0x%x", i, extMIDs[i]->m_usrAddr);
        if ((extMIDs[i]->m_render_done == true) &&
            (extMIDs[i]->m_released == true)) {
            fb->data = extMIDs[i]->m_usrAddr;
            ALOGV("1 buffer %d virtual address is 0x%x", i, extMIDs[i]->m_usrAddr);
    ...
    if (i == extMappedNativeBufferCount) {
        ALOGE("No available frame buffer in pool.");
        return -1;
    }
    return 0;
}
...
int releaseVP9FrameBuffer(void *user_priv, vpx_codec_frame_buffer_t *fb)
{
    ...
    int i;
    ALOGV("fb->data virtual address is 0x%x", fb->data);
    ...
    if (i == extMappedNativeBufferCount) {
        ALOGE("Not found matching frame buffer in pool, libvpx's wrong?");
        return -1;
    }
    return 0; 
}
...
OMX_ERRORTYPE OMXVideoDecoderVP9HWR::initDecoder()
{
    ...
    if (vpx_codec_set_frame_buffer_functions((vpx_codec_ctx_t *)mCtx,
                                    ...
                                    NULL)) {                       
      ALOGE("Failed to configure external frame buffers");    
      return OMX_ErrorNotReady;
    }
...
OMX_ERRORTYPE OMXVideoDecoderVP9HWR::ProcessorInit(void)
{
    ...
    if (NULL == module) {
        fprintf(stderr, "Hw get module failed\n");
        return OMX_ErrorUndefined;
    }
    gralloc_module = (struct gralloc_module_t*)module;
    int err = gralloc_open(module, &mAllocDev);
    if (err) {
    ...
    if (mOMXBufferHeaderTypePtrNum > MAX_NATIVE_BUFFER_COUNT) {
        ALOGE("Actual OMX outport buffer header type num (%d) > MAX_NATIVE_BUFFER_COUNT (%d)",
    ...
    ALOGV("fei: stride is %d, w is %d, h is %d\n", surfaceStride, surfaceWidth, surfaceHeight);
    ALOGV("DECODE_WITH_GRALLOC_BUFFER is defined, mOMXBufferHeaderTypePtrNum is 0x%x\n", mOMXBufferHeaderTypePtrNum);

    for (i = 0; i < mOMXBufferHeaderTypePtrNum; i++) {
        unsigned char *usrptr;
        int usage = GRALLOC_USAGE_SW_WRITE_OFTEN | GRALLOC_USAGE_SW_READ_OFTEN;
        ...
        //fei todo: need check if buf_hdr->pBuffer is native_handle_t
        gralloc_module->lock(gralloc_module, (buffer_handle_t)buf_hdr->pBuffer, usage,
                              0, 0, surfaceWidth, surfaceHeight,
                              (void **)&usrptr);
        extMIDs[i]->m_usrAddr = usrptr;
        extMappedNativeBufferCount++;
        ALOGD("fei: call gralloc->lock to get virtual address of buffer as 0x%lx\n", (unsigned long)usrptr);
                                                      ...
                                                      OMX_BOOL *isResolutionChange)
{
    ...
    if (mWorkingMode == RAWDATA_MODE) {
        if (img == NULL) {
            ALOGE("vpx_codec_get_frame return NULL.");
            return OMX_ErrorNotReady;
        }
    ...
    if (NULL != img) {
        ...
        if ((unsigned int)(buffer->pBuffer) != extMIDs[img->fb_index]->m_key) {
            ALOGE("There is gralloc handle mismatching between pool\
...
{
    ALOGV("Output color format is OMX_COLOR_FormatYUV420Planar.");
    return OMX_COLOR_FormatYUV420Planar;//HAL_PIXEL_FORMAT_YV12;
}
...
OMX_ERRORTYPE OMXVideoDecoderVP9HWR::SetNativeBufferModeSpecific(OMX_PTR pStructure)
{
    ...
    port_def.format.video.eColorFormat = OMX_COLOR_FormatYUV420Planar;//OMX_INTEL_COLOR_FormatYUV420PackedSemiPlanar;
