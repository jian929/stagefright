...
CmdProcessWork::CmdProcessWork(CmdHandlerInterface *ci)
{
    ...
    ALOGV("command process workqueue started\n");
}
...
CmdProcessWork::~CmdProcessWork()
{
    ...
    ALOGV("command process workqueue stopped\n");
}
    ...
    OMX_IN  OMX_PTR pComponentParameterStructure)
{
    ...
    case OMX_IndexExtPrepareForAdaptivePlayback: {
        ...
        if (p->nMaxFrameWidth > kMaxAdaptiveStreamingWidth
            ...
            ALOGE("resolution %d x %d exceed max driver support %d x %d\n",p->nMaxFrameWidth, p->nMaxFrameHeight,
...
void ComponentBase::CmdHandler(struct cmd_s *cmd)
{
    ...
    default:
        ALOGE("%s:%s:%s: exit failure, command %d cannot be handled\n",
             GetName(), GetWorkingRole(), GetCmdName(cmd->cmd), cmd->cmd);
        break;
    } /* switch */

    ALOGV("%s:%s: command %s handling done\n",
         GetName(), GetWorkingRole(), GetCmdName(cmd->cmd));
}
...
void ComponentBase::TransState(OMX_STATETYPE transition)
{
    ...
    ALOGV("%s:%s: try to transit state from %s to %s\n",
    ...
    if (current == transition) {
        ...
        ALOGE("%s:%s: exit failure, same state (%s)\n",
             GetName(), GetWorkingRole(), GetStateName(current));
        goto notify_event;
    }
    ...
    if (current == OMX_StateInvalid) {
        ...
        ALOGE("%s:%s: exit failure, current state is OMX_StateInvalid\n",
...
notify_event:
    if (ret == OMX_ErrorNone) {
        ...
        state = transition;
        ALOGD("%s:%s: transition from %s to %s completed",
             GetName(), GetWorkingRole(),
        ...
        if (transition == OMX_StateInvalid || ret == OMX_ErrorInvalidState) {
            ...
            ALOGE("%s:%s: exit failure, transition from %s to %s, "
                 "current state is %s\n",
                 GetName(), GetWorkingRole(), GetStateName(current),
...
inline OMX_ERRORTYPE ComponentBase::TransStateToLoaded(OMX_STATETYPE current)
{
        ...
        for (i = 0; i < nr_ports; i++)
	{
        ...
        ret = ProcessorDeinit();
        if (ret != OMX_ErrorNone) {
            ALOGE("%s:%s: ProcessorDeinit() failed "
                 "(ret : 0x%08x)\n", GetName(), GetWorkingRole(),
                 ret);
            goto out;
        }
...
inline OMX_ERRORTYPE ComponentBase::TransStateToIdle(OMX_STATETYPE current)
{
    ...
    if (current == OMX_StateLoaded) {
        ...
        if (ret != OMX_ErrorNone) {
            ALOGE("%s:%s: ProcessorInit() failed (ret : 0x%08x)\n",
        ...
        FlushPort(OMX_ALL, 0);
        pthread_mutex_unlock(&ports_block);
        ALOGV("%s:%s: flushed all ports\n", GetName(), GetWorkingRole());

        bufferwork->CancelScheduledWork(this);
        ALOGV("%s:%s: discarded all scheduled buffer process work\n",
             GetName(), GetWorkingRole());

        if (current == OMX_StatePause) {
            bufferwork->ResumeWork();
            ALOGV("%s:%s: buffer process work resumed\n",
                 GetName(), GetWorkingRole());
        }

        bufferwork->StopWork();
        ALOGV("%s:%s: buffer process work stopped\n",
             GetName(), GetWorkingRole());

        ret = ProcessorStop();
        if (ret != OMX_ErrorNone) {
            ALOGE("%s:%s: ProcessorStop() failed (ret : 0x%08x)\n",
                 GetName(), GetWorkingRole(), ret);
            goto out;
        }
    }
...
ComponentBase::TransStateToExecuting(OMX_STATETYPE current)
{
    ...
    if (current == OMX_StateIdle) {
        ...
        ALOGV("%s:%s: buffer process work started with executing state\n",
             GetName(), GetWorkingRole());

        ret = ProcessorStart();
        if (ret != OMX_ErrorNone) {
            ALOGE("%s:%s: ProcessorStart() failed (ret : 0x%08x)\n",
                 GetName(), GetWorkingRole(), ret);
            goto out;
        }
    }
        ...
        ret = ProcessorResume();
        if (ret != OMX_ErrorNone) {
            ALOGE("%s:%s: ProcessorResume() failed (ret : 0x%08x)\n",
                 GetName(), GetWorkingRole(), ret);
            goto out;
        }
    }
...
inline OMX_ERRORTYPE ComponentBase::TransStateToPause(OMX_STATETYPE current)
{
    ...
    if (current == OMX_StateIdle) {
        ...
        ALOGV("%s:%s: buffer process work started with paused state\n",
             GetName(), GetWorkingRole());

        ret = ProcessorStart();
        if (ret != OMX_ErrorNone) {
            ALOGE("%s:%s: ProcessorSart() failed (ret : 0x%08x)\n",
                 GetName(), GetWorkingRole(), ret);
            goto out;
        }
    }
        ...
        ALOGV("%s:%s: buffer process work paused\n",
             GetName(), GetWorkingRole());

        ret = ProcessorPause();
        if (ret != OMX_ErrorNone) {
            ALOGE("%s:%s: ProcessorPause() failed (ret : 0x%08x)\n",
                 GetName(), GetWorkingRole(), ret);
            goto out;
        }
    }
...
void ComponentBase::FlushPort(OMX_U32 port_index, bool notify)
{
    ...
    ALOGV("%s:%s: flush ports (from index %lu to %lu)\n",
...
void ComponentBase::TransStatePort(OMX_U32 port_index, OMX_U8 state)
{
    ...
    ALOGV("%s:%s: transit ports state to %s (from index %lu to %lu)\n",
    ...
    ALOGV("%s:%s: transit ports state to %s completed\n",
         GetName(), GetWorkingRole(), GetPortStateName(state));
}
...
OMX_ERRORTYPE ComponentBase::ApplyWorkingRole(void)
{
    ...
    ret = AllocatePorts();
    if (ret != OMX_ErrorNone) {
        ALOGE("failed to AllocatePorts() (ret = 0x%08x)\n", ret);
        return ret;
    }
    ...
    ALOGI("%s: set working role %s:", GetName(), GetWorkingRole());
    return OMX_ErrorNone;
}
...
OMX_ERRORTYPE ComponentBase::AllocatePorts(void)
{
    ...
    if (ret != OMX_ErrorNone) {
        ALOGE("failed to %s::ComponentAllocatePorts(), ret = 0x%08x\n",
             name, ret);
        return ret;
    }
...
free_ports:
    ALOGE("%s(): exit, unknown component variant\n", __func__);
    FreePorts();
    return OMX_ErrorUndefined;
}
                                       ...
                                       const buffer_retain_t *retain)
{
    ...
    if (cvariant == CVARIANT_SOURCE)
        ...
        ALOGE("%s(): fatal error unknown component variant (%d)\n",
             __func__, cvariant);
    }
}
...
OMX_ERRORTYPE ComponentBase::ProcessorProcess(OMX_BUFFERHEADERTYPE **pBuffers,
                                           ...
                                           OMX_U32 nr_buffers)
{
    ALOGE("ProcessorProcess not be implemented");
                               ...
                               bool dumpdata)
{
    ...
    ALOGD("Component %s DumpBuffer\n", name);
    ALOGD("%s port index = %lu",
    ...
    ALOGD("nAllocLen = %lu, nOffset = %lu, nFilledLen = %lu\n",
         alloc_len, offset, filled_len);
    ALOGD("nTimeStamp = %lld, nTickCount = %lu",
         bufferheader->nTimeStamp,
         bufferheader->nTickCount);
    ALOGD("nFlags = 0x%08lx\n", bufferheader->nFlags);
    ALOGD("hMarkTargetComponent = %p, pMarkData = %p\n",
