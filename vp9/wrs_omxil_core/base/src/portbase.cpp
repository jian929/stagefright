    ...
    const OMX_PARAM_PORTDEFINITIONTYPE *p, bool overwrite_readonly)
{
    ...
    if (!overwrite_readonly) {
        ...
        if ((p->nBufferSize > temp.nBufferSize) && (temp.eDir == OMX_DirInput)) {
            if (p->nBufferSize <= MAX_INPUT_PORT_SIZE) {
                ALOGW("Input port size has been changed!");
                temp.nBufferSize = p->nBufferSize;
            } else {
                ALOGE("Invalid input port size!");
                return OMX_ErrorBadParameter;
            }
        }
    }
    ...
    default:
        ALOGE("cannot find 0x%08x port domain\n", p->eDomain);
        return OMX_ErrorBadParameter;
    }
                                  ...
                                  OMX_U8 *pBuffer)
{
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu: enter, nSizeBytes=%lu\n", __FUNCTION__,
    ...
    if (portdefinition.bPopulated == OMX_TRUE) {
        ...
        ALOGV("%s(): %s:%s:PortIndex %lu: exit done, already populated\n",
    ...
    if (!buffer_hdr) {
        pthread_mutex_unlock(&hdrs_lock);
        ALOGE("%s(): %s:%s:PortIndex %lu: exit failure, "
    ...
    ALOGD("fei buffer_hdr->pAppPrivate is 0x%lx\n", (unsigned long)buffer_hdr->pAppPrivate);
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu: a buffer allocated (%p:%lu/%lu)\n",
         __FUNCTION__,
         cbase->GetName(), cbase->GetWorkingRole(), nPortIndex,
    ...
    if (nr_buffer_hdrs >= portdefinition.nBufferCountActual) {
        ...
        ALOGV("%s(): %s:%s:PortIndex %lu: allocate all buffers (%lu)\n",
             __FUNCTION__, cbase->GetName(), cbase->GetWorkingRole(),
    ...
    *ppBufferHdr = buffer_hdr;
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu: exit done\n", __FUNCTION__,
         cbase->GetName(), cbase->GetWorkingRole(), nPortIndex);
    return OMX_ErrorNone;
}
                                   ...
                                   OMX_BUFFERHEADERTYPE *pBuffer)
{
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu:pBuffer %p: enter\n", __FUNCTION__,
    ...
    if (!entry) {
        pthread_mutex_unlock(&hdrs_lock);
        ALOGE("%s(): %s:%s:PortIndex %lu:pBuffer %p: exit failure, "
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu:pBuffer %p: free a buffer (%lu/%lu)\n",
    ...
    if (!nr_buffer_hdrs) {
        buffer_hdrs_completion = true;
        pthread_cond_signal(&hdrs_wait);
        ALOGV("%s(): %s:%s:PortIndex %lu: free all allocated buffers (%lu)\n",
             __FUNCTION__, cbase->GetName(), cbase->GetWorkingRole(),
             nPortIndex, portdefinition.nBufferCountActual);
    }
...
void PortBase::WaitPortBufferCompletion(void)
{
    ...
    if (!buffer_hdrs_completion) {
        ALOGV("%s(): %s:%s:PortIndex %lu: wait for buffer header completion\n",
             __FUNCTION__, cbase->GetName(), cbase->GetWorkingRole(),
             portdefinition.nPortIndex);
        pthread_cond_wait(&hdrs_wait, &hdrs_lock);
        ALOGV("%s(): %s:%s:PortIndex %lu: wokeup (buffer header completion)\n",
             __FUNCTION__, cbase->GetName(), cbase->GetWorkingRole(),
             portdefinition.nPortIndex);
    }
...
OMX_ERRORTYPE PortBase::WaitPortBufferCompletionTimeout(int64_t milliseconds)
{
    ...
    if (rc == ETIMEDOUT) {
        ALOGE("%s(): %s:%s:PortIndex %lu: wokeup (buffer header timeout)\n",
             __FUNCTION__, cbase->GetName(), cbase->GetWorkingRole(),
             portdefinition.nPortIndex);
...
OMX_ERRORTYPE PortBase::PushThisBuffer(OMX_BUFFERHEADERTYPE *pBuffer)
{
    ...
    ALOGV_IF(pBuffer != NULL, "%s(): %s:%s:PortIndex %lu:pBuffer %p:\n",
            __FUNCTION__, cbase->GetName(), cbase->GetWorkingRole(),
...
OMX_BUFFERHEADERTYPE *PortBase::PopBuffer(void)
{
    ...
    ALOGV_IF((buffer != NULL || RetainedBufferQueueLength() > 0), "%s(): %s:%s:PortIndex %lu:pBuffer %p:\n",
            __FUNCTION__, cbase->GetName(), cbase->GetWorkingRole(),
...
OMX_ERRORTYPE PortBase::ReturnThisBuffer(OMX_BUFFERHEADERTYPE *pBuffer)
{
    ...
    if (pBuffer->nFlags & OMX_BUFFERFLAG_EOS) {
        ALOGV("%s(): %s:%s:PortIndex %lu:pBuffer %p: "
        ...
        bool accumulate)
{
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu:pBuffer %p: enter, %s\n", __FUNCTION__,
         cbase->GetName(), cbase->GetWorkingRole(), portdefinition.nPortIndex,
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu:pBuffer %p: exit done\n", __FUNCTION__,
         cbase->GetName(), cbase->GetWorkingRole(),
         portdefinition.nPortIndex, pBuffer);
    return OMX_ErrorNone;
}
...
void PortBase::ReturnAllRetainedBuffers(void)
{
    ...
    do {
        ...
        if (buffer) {
            ALOGV("%s(): %s:%s:PortIndex %lu: returns a retained buffer "
            ...
            if (ret != OMX_ErrorNone)
                ALOGE("%s(): %s:%s:PortIndex %lu: failed (ret : 0x%x08x)\n",
                     __FUNCTION__,
                     cbase->GetName(), cbase->GetWorkingRole(),
...
OMX_ERRORTYPE PortBase::FlushPort(void)
{
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu: enter\n", __FUNCTION__,
         cbase->GetName(), cbase->GetWorkingRole(),
         portdefinition.nPortIndex);
...
OMX_ERRORTYPE PortBase::TransState(OMX_U8 transition)
{
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu: enter, transition from %s to %s\n",
         __FUNCTION__,
         cbase->GetName(), cbase->GetWorkingRole(), portdefinition.nPortIndex,
    ...
    if (current == transition) {
        ...
        ALOGE("%s(): %s:%s:PortIndex %lu: exit failure, same state (%s)\n",
             __FUNCTION__,
             cbase->GetName(), cbase->GetWorkingRole(),
    ...
    if (transition == OMX_PortEnabled) {
        ...
        ret = OMX_ErrorBadParameter;
        ALOGE("%s(): %s:%s:PortIndex %lu: exit failure, invalid transition "
             "(%s)\n", __FUNCTION__,
             cbase->GetName(), cbase->GetWorkingRole(),
    ...
    ALOGV("%s(): %s:%s:PortIndex %lu: transition from %s to %s complete\n",
         __FUNCTION__,
         cbase->GetName(), cbase->GetWorkingRole(), portdefinition.nPortIndex,
...
OMX_ERRORTYPE PortBase::ReportEvent(OMX_EVENTTYPE eventType)
{
    ...
    if (eventType < 0 || eventType > OMX_EventMax) {
        ALOGE("invalid parameters for EventHandler!");
        return OMX_ErrorBadParameter;
    }
