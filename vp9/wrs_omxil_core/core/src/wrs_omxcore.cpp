...
OMX_API OMX_ERRORTYPE OMX_APIENTRY OMX_Init(void)
{
    ...
    if (!g_initialized) {
        ...
        if (!g_module_list) {
            ...
            ALOGE("%s(): exit failure, construct_components failed",
                 __FUNCTION__);
    ...
    ALOGV("%s(): exit done", __FUNCTION__);
...
OMX_API OMX_ERRORTYPE OMX_APIENTRY OMX_Deinit(void)
{
    ...
    ALOGV("%s(): exit done (ret : 0x%08x)", __FUNCTION__, ret);
    return ret;
}
    ...
    OMX_IN OMX_U32 nIndex)
{
    ...
    ALOGV("%s(): found %luth component %s", __FUNCTION__, nIndex, cname);
    return OMX_ErrorNone;
}
    ...
    OMX_IN OMX_CALLBACKTYPE *pCallBacks)
{
    ...
    ALOGV("%s(): enter, try to get %s", __FUNCTION__, cComponentName);
    ...
    list_foreach(g_module_list, entry) {
        ...
        if (!strcmp(cComponentName, cname)) {
            ...
            if (ret != OMX_ErrorNone) {
                ALOGE("%s(): exit failure, cmodule->Load failed\n",
                     __FUNCTION__);
                goto unlock_list;
            }
            ...
            if (ret != OMX_ErrorNone){
                ALOGE("%s(): exit failure, cmodule->Instantiate failed\n",
                     __FUNCTION__);
                goto unload_cmodule;
            }
            ...
            if (ret != OMX_ErrorNone) {
                ALOGE("%s(): exit failure, cbase->GetHandle failed\n",
                     __FUNCTION__);
                goto delete_cbase;
            }
            ...
            ALOGI("get handle of component %s successfully", cComponentName);
            ALOGV("%s(): exit done\n", __FUNCTION__);
            return OMX_ErrorNone;
        ...
        unlock_list:
            ...
            ALOGE("%s(): exit failure, (ret : 0x%08x)\n", __FUNCTION__, ret);
            return ret;
        }
    }
    pthread_mutex_unlock(&g_module_lock);

    ALOGE("%s(): exit failure, %s not found", __FUNCTION__, cComponentName);
    ...
    OMX_IN OMX_HANDLETYPE hComponent)
{
    ...
    if (!cbase) {
        ALOGE("%s(): exit failure, cannot find cbase pointer\n",
             __FUNCTION__);
        return OMX_ErrorBadParameter;
    }
    strcpy(&cname[0], cbase->GetName());

    ALOGV("%s(): enter, try to free %s", __FUNCTION__, cbase->GetName());


    ret = cbase->FreeHandle(hComponent);
    if (ret != OMX_ErrorNone) {
        ALOGE("%s(): exit failure, cbase->FreeHandle() failed (ret = 0x%08x)\n",
             __FUNCTION__, ret);
        return ret;
    }
    ...
    if (!cmodule)
        ALOGE("fatal error, %s does not have cmodule\n", cbase->GetName());
    ...
    ALOGI("free handle of component %s successfully", cname);
    ALOGV("%s(): exit done", __FUNCTION__);
    return OMX_ErrorNone;
}
    ...
    list_foreach(g_module_list, entry) {
        ...
        if (having_role) {
            if (compNames && compNames[nr_comps]) {
                ...
                copied_nr_comps++;
                ALOGV("%s(): component %s has %s role", __FUNCTION__,
                     cname, role);
            }
            nr_comps++;
        }
